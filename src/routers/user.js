...
const multer = Multer.diskStorage({
    destination: (req, file, cb) => {
        // The line below was supposed to be used for changing your profile picture. So I delete it first. This is incomplete because the other places where we need to save it is not saving. Like I didn't code it to do it. Wanted to finish this project so i can start learning native android development to help carter with his college project.
        if(fs.existsSync("images/" + file.originalname)){
...
// I was lazy to move the upload middle ware to sign up route handler. had to refactor some code in order to accomplish that.
router.post('/users/upload-profile-picture', upload.single('image'), async (req, res) => {
    ...
    // form.parse(req, async (err, fields, files) => {
      try { 
        ...
        // console.log("here is the value of the fields object ", fields)

        // console.log("1 ", files.image)
        // console.log("2 ", files)
        ...
        res.send({ message: "we successfully uploaded the user profile picture!" })
    } catch (e) {
...
router.get('/users/profile-picture', async (req, res) => {
    try{
        console.log("we made it in the profile picture route")
        res.sendFile(path.join(__dirname, "./../../images/" + req.query.file_name));
    } catch (e) {
      res.status(400).send({ error: e.message })
      console.log("here is the error message ", e.message)
...
router.post('/users', async (req, res) => {
    try{
        ...
        // Generate the list of chat IDs for each 1 to 1 direct messenging for the user that is being created right now
        for(let existing_user of get_list_of_all_current_users){
            


            // Check if we are looping through the user that was just created. if so then don't create a chat room because it doesn't make sense to be able to send a message to your self.
            // if(user.id !== existing_user.id){ // I have no fucking clue why im checking this. i doesn't make sense. just left it just in case.
            
            
            // -- IMPORTANT -- 
            // You only have to check if first_user_ID and second_user_ID is swapped due to it being an existing user when creating another user account that made the existing user id set to the second_user_ID column instead of the first. so, when trying to get the previous messages for a chat room
            // is the only time you have to check if first_user_ID and second_user_ID in the chat table is swapped when trying to get the chat ID to get the list of past chat messages for a 1 on 1 chat room. I know it sounds really complicated and i hope you understand what im talking about if not just trust what im saying and do as i say. this is thought through.
            const new_chat_for_this_user = new Chat({ 
                    first_user_ID: user.id,
                    second_user_ID: existing_user.id
                })

                await new_chat_for_this_user.save();
            // }
...
router.post('/users/login', async (req, res) => {
        try{
            ...
            console.log("email" + req.body.email + "hi") 
            console.log("password" + req.body.password + "hi")
            const user = await User.findOne({where: {email: req.body.email.trim()}}) //Look for the user profile by the email they entered when logging in
...
router.post('/users/get-profile-picture-for-right-drawer', async (req, res) => {
    try{
        ...
        // I have to do this check because if there is only one user online or in total then it will be an array if more than one then it will be inside a object property called online_users. Don't know exactly why this happens but not a big deal and no point in understanding why because there is no negative side effects from doing it this way. It will save me time to finishing other task. WORK SMART NOT HARD is the key lesson here.
        if(Array.isArray(req.body.online_users_names)){
            ...
            for(const online_user_name of req.body.online_users_names.online_users){
                let get_online_users_with_profile_picture = await User.findOne({
                    where: {
                        name: online_user_name // Don't worry the user name has to be unique in the database table so there wont be any conflict.
                    }
...
router.get('/users', async (req, res) => {
    try{
        ...
        // Did not work. Do it in the front end.
        // let users_with_their_profile_pic = [];
        // for(let user of all_users_on_this_app){
        //     var storage = firebase.storage();
        //    let downloadedImage = await storage.child(user['profile_pic']).getDownloadURL()
        //    user.downloadedeImage = downloadedImage
        //    users_with_their_profile_pic.push(user);
        // }


        // res.send({users_with_their_profile_pic})
