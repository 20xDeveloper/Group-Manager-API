...
const project_section_router = require('./routers/project_section')
const task_router = require('./routers/task')
const task_update_router = require('./routers/task_update')
...
app.use(project_section_router);
app.use(task_router);
app.use(task_update_router);
...
io.on('connection', async (socket) => {
  ...
  socket.on('user started messaging', async (both_users_ID) => {

    console.log("here is the value for both_users_ID " + both_users_ID)
    ...
    let chat_ID_for_channel_name = await Chat.findOne({
      where: {
        first_user_ID: both_users_ID[0],
        second_user_ID: both_users_ID[1]
      }
    })
    ...
    io.emit('recieve past messages for this conversation', past_messages_with_username)
  })
  ...
  socket.on('broadcast user message', async (user_message, username, channel_ID, image) => {
    try {
      // -- Create the message record --
      console.log("we made it to the broadcast event.")
      console.log("here is the value of the user_message " + user_message.toString())
      console.log("here is the value of the username " + username.toString())
      console.log("here is the value of the channel_ID " + channel_ID.toString())
      ...
      console.log("found the user " + user_ID_for_this_message.toString())
      ...
      console.log("found the chat room " + chat_ID_for_this_message.toString())
